...
export const opportunitiesApi = {
  ...
  create: async (payload: Partial<Opportunity>) => {
    const { data } = await httpClient.post<Opportunity>('/opportunities', payload);
    return data;
  },
  update: async (id: string, payload: Partial<Opportunity>) => {
    const { data } = await httpClient.patch<Opportunity>(`/opportunities/${id}`, payload);
    return data;
  },
  updateStage: async (id: string, stage: string) => {
    const { data } = await httpClient.patch<Opportunity>(`/opportunities/${id}/stage`, { stage });
    return data;
  },
