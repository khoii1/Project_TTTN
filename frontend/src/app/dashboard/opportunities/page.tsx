...
export default function OpportunitiesListPage() {
  ...
  const fetchOpportunities = async (page: number, limit: number, search: string, stage?: string) => {
    try {
      setLoading(true);
      ...
      if (oppsRes && oppsRes.data && Array.isArray(oppsRes.data)) {
        setOpportunities(oppsRes.data);
        setTotal(oppsRes.meta?.total || 0);
      } else if (Array.isArray(oppsRes)) {
        setOpportunities(oppsRes);
        setTotal(oppsRes.length);
      } else {
        setOpportunities([]);
        setTotal(0);
      }
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'stage',
      key: 'stage',
      render: (stage: string) => <Tag color={stageColors[stage] || 'default'}>{stage}</Tag>,
    },
    {
      title: 'Close Date',
      dataIndex: 'closeDate',
    ...
    },
    {
      ...
      render: (_: any, record: Opportunity) => (
        <Space size="middle">
          <Button type="text" icon={<EyeOutlined />} onClick={() => router.push(`/dashboard/opportunities/${record.id}`)} />
          <Popconfirm title="Delete this opportunity?" onConfirm={() => handleDelete(record.id)}>
  ...
  return (
    <div>
      <PageHeader 
        title="Opportunities" 
        action={
          <Button type="primary" icon={<PlusOutlined />} onClick={() => router.push('/dashboard/opportunities/new')}>
            New Opportunity
          </Button>
        }
      />
      <div className="mb-4 flex space-x-4">
        ...
        <Select 
          ...
          className="w-48"
        >
          {Object.values(OpportunityStage).map(s => <Select.Option key={s} value={s}>{s}</Select.Option>)}
        </Select>
      </div>
      <Table 
        columns={columns} 
