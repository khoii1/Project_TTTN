...
import { Card, Descriptions, Spin, message, Button, Steps, Form, Input, Select } from 'antd';
      ...
      <Card className="shadow-sm">
        ...
        {isEditing ? (
          <Form form={form} layout="vertical" onFinish={handleUpdate}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Form.Item name="priority" label="Priority">
                <Select>
                  {Object.values(CasePriority).map(p => <Select.Option key={p} value={p}>{p}</Select.Option>)}
                </Select>
              </Form.Item>
              <Form.Item name="accountId" label="Account">
                <Select placeholder="Select an Account" showSearch optionFilterProp="children" allowClear>
                  {accounts.map(acc => (
          ...
          <Descriptions column={2} bordered size="middle">
            <Descriptions.Item label="Subject" span={2}>{caseItem.subject}</Descriptions.Item>
            <Descriptions.Item label="Status">{caseItem.status}</Descriptions.Item>
            <Descriptions.Item label="Priority">{caseItem.priority}</Descriptions.Item>
            <Descriptions.Item label="Account">
               {accounts.find(a => a.id === caseItem.accountId)?.name || 'N/A'}
            </Descriptions.Item>
            <Descriptions.Item label="Created">{new Date(caseItem.createdAt).toLocaleString()}</Descriptions.Item>
            <Descriptions.Item label="Description" span={2}>{caseItem.description || 'N/A'}</Descriptions.Item>
          </Descriptions>
        )}
      </Card>
