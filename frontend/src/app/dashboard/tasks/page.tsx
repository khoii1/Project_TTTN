...
export default function TasksListPage() {
  ...
  const fetchTasks = async (page: number, limit: number, search: string, status?: string, priority?: string) => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    fetchTasks(currentPage, currentLimit, currentSearch, currentStatus, currentPriority);
  ...
  const columns = [
    {
      ...
      render: (text: string, record: Task) => (
        <span className={`font-medium ${record.status === 'COMPLETED' ? 'line-through text-gray-400' : ''}`}>
          {text}
        </span>
      ),
    },
      ...
      render: (_: any, record: Task) => (
        <Space size="middle">
          {record.status !== 'COMPLETED' && (
            <Button type="text" icon={<CheckOutlined className="text-green-500" />} onClick={() => handleComplete(record.id)} />
          )}
          <Button type="text" icon={<EyeOutlined />} onClick={() => router.push(`/dashboard/tasks/${record.id}`)} />
  ...
  return (
    <div>
      <PageHeader 
        title="Tasks" 
        action={
          <Button type="primary" icon={<PlusOutlined />} onClick={() => router.push('/dashboard/tasks/new')}>
            New Task
          </Button>
        }
      />
