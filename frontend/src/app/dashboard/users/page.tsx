  ...
  const columns = [
    {
      title: 'Name',
      key: 'name',
      render: (_: any, record: User) => <span className="font-medium">{record.firstName} {record.lastName}</span>,
    },
    {
      title: 'Email',
      dataIndex: 'email',
    ...
    <div>
      <PageHeader 
        ...
        action={
          currentUser?.role === 'ADMIN' ? (
            <Button type="primary" icon={<PlusOutlined />} onClick={() => router.push('/dashboard/users/new')}>
              New User
            </Button>
          ) : null
        }
      ...
      <div className="mb-4 flex space-x-4">
        ...
        <Select 
          ...
          className="w-48"
        >
          {Object.values(UserRole).map(s => <Select.Option key={s} value={s}>{s}</Select.Option>)}
        </Select>
      </div>
      <Table 
        columns={columns} 
