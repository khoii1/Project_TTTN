  ...
  const columns = [
    {
      title: 'Name',
      key: 'name',
      render: (_: any, record: Contact) => <span className="font-medium">{record.firstName} {record.lastName}</span>,
    },
    {
      title: 'Title',
      dataIndex: 'title',
    ...
    },
    {
      ...
      render: (_: any, record: Contact) => (
        <Space size="middle">
          <Button type="text" icon={<EyeOutlined />} onClick={() => router.push(`/dashboard/contacts/${record.id}`)} />
          <Popconfirm title="Delete this contact?" onConfirm={() => handleDelete(record.id)}>
  ...
  return (
    <div>
      <PageHeader 
        title="Contacts" 
        action={
          <Button type="primary" icon={<PlusOutlined />} onClick={() => router.push('/dashboard/contacts/new')}>
            New Contact
          </Button>
        }
      />
